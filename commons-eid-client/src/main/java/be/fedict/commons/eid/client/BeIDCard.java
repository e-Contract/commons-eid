...
public class BeIDCard {
	...
	private static final byte[] BELPIC_AID = new byte[] { (byte) 0xA0, 0x00, 0x00, 0x01, 0x77, 0x50, 0x4B, 0x43, 0x53,
			0x2D, 0x31, 0x35, };
	private static final byte[] APPLET_AID = new byte[] { (byte) 0xA0, 0x00, 0x00, 0x00, 0x30, 0x29, 0x05, 0x70, 0x00,
	 ...
	 * 
	 * @param card   a javax.smartcardio.Card that you have previously determined to
	 *               be a BeID Card
	 * @param logger an instance of be.fedict.commons.eid.spi.Logger
	 * @throws IllegalArgumentException when passed a null logger. to disable
	 *                                  logging, call BeIDCard(Card) instead.
	 * @throws RuntimeException         when no CertificateFactory capable of
	 *                                  producing X509 Certificates is available.
	 */
	 ...
	 * @throws RuntimeException         when no CertificateFactory capable of
	 *                                  producing X509 Certificates is available.
	 * @throws CardException            in case of a smart card I/O error.
	 */
	...
	/**
	 * Returns the X509 authentication certificate. This is a convenience method for
	...
	public X509Certificate getAuthenticationCertificate()
			throws CardException, IOException, CertificateException, InterruptedException {
	...
	/**
	 * Returns the national registration certificate. This is a convencience method
	 * for <code>getCertificate(FileType.RRNCertificate)</code>
	 * 
	 * @return
	 * @throws CardException
	 * @throws IOException
	 ...
	 * file types corresponding with a certificate are accepted. Which certificate's
	 * chain to return is determined by the FileType param. Applicable FileTypes are
	 * AuthentificationCertificate, NonRepudiationCertificate, CACertificate, and
	 * RRNCertificate.
	 * 
	...
	public List<X509Certificate> getCertificateChain(final FileType fileType)
		...
		if (fileType.chainIncludesCitizenCA()) {
			chain.add((X509Certificate) this.certificateFactory
					.generateCertificate(new ByteArrayInputStream(readFile(FileType.CACertificate))));
		}
		chain.add((X509Certificate) this.certificateFactory
				.generateCertificate(new ByteArrayInputStream(readFile(FileType.RootCertificate))));
		return chain;
	}
	...
	/**
	 * Returns the X509 authentication certificate chain. (Authentication -> Citizen
	 * CA -> Root) This is a convenience method for
	...
	/**
	 * Returns the Citizen CA X509 certificate chain. (Citizen CA -> Root) This is a
	 ...
	 * @param digestAlgo          the algorithm used to calculate the given digest
	 *                            value.
	 * @param fileType            the certificate's file type.
	 * @param requireSecureReader <code>true</code> if a secure pinpad reader is
	 *                            required.
	 * @return
	 * @throws CardException
	 * @throws IOException
	...
	public byte[] sign(final byte[] digestValue, final BeIDDigest digestAlgo, final FileType fileType,
			final boolean requireSecureReader)
			throws CardException, IOException, InterruptedException, UserCancelledException {
		return sign(digestValue, digestAlgo, fileType, requireSecureReader, null);
	}
	...
	public byte[] sign(final byte[] digestValue, final BeIDDigest digestAlgo, final FileType fileType,
		...
		try {
			...
			ResponseAPDU responseApdu = transmitCommand(BeIDCommandAPDU.SELECT_ALGORITHM_AND_PRIVATE_KEY,
			...
			/*
			 * 0x6982 = Security status not satisfied, so we do a PIN verification before
			 * retrying.
			 */
	 ...
	 * @param requireSecureReader whether to require a secure pinpad reader to
	 ...
	 * @throws UserCancelledException
	 */
	public byte[] signAuthn(final byte[] toBeSigned, final boolean requireSecureReader)
	 ...
	 * @throws UserCancelledException
	 */
	public byte[] signAuthn(final byte[] toBeSigned, final boolean requireSecureReader, final String applicationName)
			throws NoSuchAlgorithmException, CardException, IOException, InterruptedException, UserCancelledException {
	 ...
	 * a successfully tests PIN into the PIN cache, so that unless the card is
	 * removed, a subsequent authentication attempt will not request the PIN, but
	 * proceed with the PIN given here.
	 * 
	 * @throws IOException
	 * @throws CardException
	 * @throws InterruptedException
	...
	/**
	 * Change PIN code. This method will attempt to change PIN using the most secure
	 * method available. if requiresSecureReader is true, this will throw a
	...
	public byte[] getChallenge(final int size) throws CardException {
		...
		if (0x9000 != responseApdu.getSW()) {
			...
			throw new ResponseAPDUException("get challenge failure: " + Integer.toHexString(responseApdu.getSW()),
					responseApdu);
		}
	 ...
	 * pinpad readers can visualize such type of text message transactions on their
	 * hardware display.
	 * 
	...
	public byte[] signTransactionMessage(final String transactionMessage, final boolean requireSecureReader)
			throws CardException, IOException, InterruptedException, UserCancelledException {
		return signTransactionMessage(transactionMessage, requireSecureReader, null);
	}
	...
	public byte[] signTransactionMessage(final String transactionMessage, final boolean requireSecureReader,
		...
		byte[] signature;
		try {
			signature = this.sign(transactionMessage.getBytes(), BeIDDigest.PLAIN_TEXT,
					FileType.AuthentificationCertificate, requireSecureReader, applicationName);
		} finally {
			if (getCCID().hasFeature(CCID.FEATURE.EID_PIN_PAD_READER)) {
	 ...
	 * Authentication signatures will require PIN entry. (non-repudation signatures
	 ...
	 * to unblock a blocked PIN. If requireSecureReader is true, will throw
	 * SecurityException if an SPR is not available
	 * 
	 * @param requireSecureReader
	 * @throws Exception
	 */
	 ...
	 * constructed using the CardReader constructor, this is the only way to get to
	 * the ATR.
	 * 
	 * @return
	 */
	 ...
	 * the Locale of any explicitly set UI, as well. BeIDCard instances, while using
	 * the global Locale settings made in BeIDCards and/or BeIDCardManager by
	 * default, may have their own individual Locale settings that may override
	...
	/**
	 * Select the BELPIC applet on the chip. Since the BELPIC applet is supposed to
	 * be all alone on the chip, shouldn't be necessary.
	 * 
	 * @return this BeIDCard instance, to allow method chaining
	 * @throws CardException
	 */
	 ...
	 * endExclusive(). Other threads will receive a CardException. Use this when you
	 * need to make several calls to the card that depend on each other. for
	 ...
	 * 
	 * Called automatically by the higher-level methods in this class. If you end up
	 * calling this directly, this is either something wrong with your code, or with
	 * this class. Please let us know. You should really only have to be calling
	 * this when using some of the other low-level methods (transmitCommand, etc..)
	 * *never* in combination with the high-level methods.
	 * 
	 * @return this BeIDCard Instance, to allow method chaining.
	 * @throws CardException
	 */
	public BeIDCard beginExclusive() throws CardException {
		...
		if (this.exclusiveAccessThread != null) {
			throw new IllegalStateException(
					"Exclusive access already granted to " + this.exclusiveAccessThread.getName());
		}
	 ...
	 * by a call to selectFile so the card knows what you want to read. Consider
	 * using one of the higher-level methods, or readFile().
	 * 
	 * @param fileType         the file to read (to allow for notification)
	 * @param estimatedMaxSize the estimated total size of the file to read (to
	...
	public byte[] readBinary(final FileType fileType, final int estimatedMaxSize)
		...
		do {
			...
			final ResponseAPDU responseApdu = transmitCommand(BeIDCommandAPDU.READ_BINARY, offset >> 8, offset & 0xFF,
					BLOCK_SIZE);
			final int sw = responseApdu.getSW();
			if (0x6B00 == sw) {
				/*
				 * Wrong parameters (offset outside the EF) End of file reached. Can happen in
				 * case the file size is a multiple of 0xff bytes.
				 */
				break;
			}
	...
	/**
	 * Gives back the card data as byte array. For Belpic applet version 1.8 eID
	 * cards, this will return the extended version.
	 * 
	 * @return
	 * @throws CardException
	 * @throws FileNotFoundException
	...
	public byte[] getCardData() throws CardException, FileNotFoundException {
		...
		ResponseAPDU responseApdu = transmitCommand(apdu, 0xff);
		if (0x9000 != responseApdu.getSW()) {
	...
	protected byte[] transmitCCIDControl(final boolean usePPDU, final CCID.FEATURE feature, final byte[] command)
			throws CardException {
		if (usePPDU) {
	...
	protected ResponseAPDU transmitCommand(final BeIDCommandAPDU apdu, final int p2, final byte[] data)
			throws CardException {
	...
	protected ResponseAPDU transmitCommand(final BeIDCommandAPDU apdu, final byte[] data, final int dataOffset,
			final int dataLength, final int ne) throws CardException {
		return transmit(new CommandAPDU(apdu.getCla(), apdu.getIns(), apdu.getP1(), apdu.getP2(), data, dataOffset,
				dataLength, ne));
	}
	...
	private ResponseAPDU transmit(final CommandAPDU commandApdu, final int attempt) throws CardException {
		if (attempt >= 32) {
		...
		if (0x6c == responseApdu.getSW1()) {
			 ...
			 * next BeIDCommandAPDU is mandatory for eID v1.0 and v1.1 cards.
			 */
			...
			CommandAPDU newCommandApdu = new CommandAPDU(commandApdu.getCLA(), commandApdu.getINS(),
					commandApdu.getP1(), commandApdu.getP2(), commandApdu.getData(), responseApdu.getSW2());
			...
			CommandAPDU newCommandApdu = new CommandAPDU(0x00, 0xC0, 0x00, 0x00, le);
	...
	/*
	 * Verify PIN code for purpose "purpose" This method will attempt to verify PIN
	 * using the most secure method available. If that method turns out to be the
	 * UI, will pass purpose to the UI.
	 */

	private void verifyPin(final PINPurpose purpose, final String applicationName)
			throws IOException, CardException, InterruptedException, UserCancelledException {
		ResponseAPDU responseApdu;
		int retriesLeft = -1;
		do {
			if (getCCID().hasFeature(CCID.FEATURE.VERIFY_PIN_DIRECT)) {
				responseApdu = verifyPINViaCCIDDirect(retriesLeft, purpose, applicationName);
			} else if (getCCID().hasFeature(CCID.FEATURE.VERIFY_PIN_START)) {
				responseApdu = verifyPINViaCCIDStartFinish(retriesLeft, purpose, applicationName);
			} else {
				responseApdu = verifyPINViaUI(retriesLeft, purpose, applicationName);
			}
	...
	private ResponseAPDU verifyPINViaCCIDDirect(final int retriesLeft, PINPurpose purpose, String applicationName)
	...
	private ResponseAPDU verifyPINViaCCIDStartFinish(final int retriesLeft, PINPurpose purpose, String applicationName)
	...
	private ResponseAPDU verifyPINViaUI(final int retriesLeft, final PINPurpose purpose, final String applicationName)
			throws CardException, UserCancelledException {
		final boolean windows = this.isWindows();
		if (windows) {
			this.endExclusive();
		}
		final char[] pin = getUI().obtainPIN(retriesLeft, purpose, applicationName);
		if (windows) {
			this.beginExclusive();
		}
		final byte[] verifyData = new byte[] { (byte) (0x20 | pin.length), (byte) 0xFF, (byte) 0xFF, (byte) 0xFF,
	...
	private ResponseAPDU changePINViaUI(final int retriesLeft) throws CardException {
		...
		final byte[] changePinData = new byte[] { (byte) (0x20 | oldPin.length), (byte) 0xFF, (byte) 0xFF, (byte) 0xFF,
	...
	private ResponseAPDU unblockPINViaCCIDVerifyPINDirectOfPUK(final int retriesLeft)
			throws IOException, CardException {
	...
	private ResponseAPDU unblockPINViaUI(final int retriesLeft) throws CardException {
		...
		final byte[] unblockPinData = new byte[] { (byte) (0x20 | ((byte) (puk1.length + puk2.length))), (byte) 0xFF,
				(byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, };
	...
	/*
	 * BeIDCommandAPDU encapsulates values sent in CommandAPDU's, to make these more
