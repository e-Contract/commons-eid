...
public class CCID {
	...
	public static final Set<String> ppduNames = new HashSet<>();
	...
	public CCID(final Card card, final CardTerminal cardTerminal, final Logger logger) {
		...
		try {
			...
			if (this.features.isEmpty() && onMSWindows && isPPDUCardTerminal(cardTerminal)) {
			...
			if (onMSWindows && isPPDUCardTerminal(cardTerminal)) {
				...
				this.logger.debug("Not risking PPDU Fallback strategy for CardTerminal [" + cardTerminal.getName()
						+ "] on this platform");
			}
		}
	}
	...
	private void getFeaturesUsingControlChannel(final Card card, final boolean onMSWindows) throws CardException {
		byte[] featureBytes = card.transmitControlCommand(onMSWindows ? GET_FEATURES_MICROSOFT : GET_FEATURES,
				new byte[0]);
	...
	private void getFeaturesUsingPPDU(final Card card) throws CardException {
		ResponseAPDU responseAPDU = card.getBasicChannel()
	...
	protected byte[] transmitPPDUCommand(final int controlCode, final byte[] command) throws CardException {
		ResponseAPDU responseAPDU = card.getBasicChannel()
				.transmit(new CommandAPDU((byte) 0xff, (byte) 0xc2, 0x01, controlCode, command));
		if (responseAPDU.getSW() != 0x9000) {
	...
	public void waitForOK() throws CardException, InterruptedException {
		...
		loop: while (true) {
			final byte[] getKeyPressedResult = transmitControlCommand(this.getFeature(FEATURE.GET_KEY_PRESSED),
					new byte[0]);
	...
	public byte[] createPINVerificationDataStructure(final Locale locale, final INS ins) throws IOException {
		...
		verifyCommand.write(0x80 | 0x08 | 0x01); // bmFormatString
		/*
		 * bmFormatString. bit 7: 1 = system units are bytes
		 * 
		 * bit 6-3: 1 = PIN position in APDU command after Lc, so just after the 0x20 |
		 * pinSize.
		 * 
		...
		/*
		 * 0x02 = validation key pressed. So the user must press the green button on his
		 * pinpad.
		 */
		 ...
		 */
		final byte[] verifyApdu = new byte[] { 0x00, // CLA
				(byte) ins.getIns(), // INS
				0x00, // P1
				0x01, // P2
	...
	public byte[] createPINModificationDataStructure(final Locale locale, final INS ins) throws IOException {
		...
		modifyCommand.write(0x80 | 0x08 | 0x01); // bmFormatString
		/*
		 * bmFormatString. bit 7: 1 = system units are bytes
		 * 
		 * bit 6-3: 1 = PIN position in APDU command after Lc, so just after the 0x20 |
		...
		final byte[] modifyApdu = new byte[] { 0x00, // CLA
				(byte) ins.getIns(), // INS
				0x00, // P1
				0x01, // P2
