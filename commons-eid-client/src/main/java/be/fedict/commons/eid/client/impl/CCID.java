...
public class CCID
{
  ...
  public enum FEATURE
  {
    VERIFY_PIN_START(0x01),VERIFY_PIN_FINISH(0x02),VERIFY_PIN_DIRECT(0x06),MODIFY_PIN_START(0x03),MODIFY_PIN_FINISH(
      0x04),MODIFY_PIN_DIRECT(0x07),GET_KEY_PRESSED(0x05),EID_PIN_PAD_READER(0x80);
  ...
  {
    ResponseAPDU responseAPDU=card.getBasicChannel().transmit(new CommandAPDU((byte)0xff,(byte)0xc2,0x01,controlCode,command,32));
    if(responseAPDU.getSW()!=0x9000)
  ...
  public void waitForOK() throws CardException,InterruptedException
  {
    ...
    while(true)
    {
      final byte[] getKeyPressedResult=transmitControlCommand(this.getFeature(FEATURE.GET_KEY_PRESSED),new byte[0]);
  ...
  public byte[] createPINModificationDataStructure(final Locale locale,final INS ins) throws IOException
  {
    ...
    final byte[] modifyApdu=new byte[]{
      ...
      (byte)0x20,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0x20,(byte)0xFF,
